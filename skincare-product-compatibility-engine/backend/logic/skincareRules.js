...
export const skinTypeRules = {
  dry: {
    avoid: ["alcohol", "alcohol denat", "sd alcohol", "denatured alcohol", "salicylic acid", "benzoyl peroxide", "clay", "charcoal", "sulfates", "sodium lauryl sulfate"],
    prefer: ["hyaluronic acid", "ceramides", "glycerin", "squalane", "shea butter", "jojoba oil", "vitamin e", "aloe vera", "niacinamide", "panthenol"],
    description: "Dry skin needs hydration and moisture barrier support."
  },
  oily: {
    avoid: ["mineral oil", "coconut oil", "cocoa butter", "lanolin", "petroleum", "isopropyl myristate", "heavy oils", "silicone", "dimethicone"],
    prefer: ["salicylic acid", "niacinamide", "hyaluronic acid", "tea tree oil", "zinc", "glycolic acid", "retinol", "clay", "charcoal"],
    description: "Oily skin benefits from oil control and pore cleansing."
  },
  combination: {
    avoid: ["heavy oils", "alcohol denat", "sodium lauryl sulfate", "mineral oil", "cocoa butter"],
    prefer: ["niacinamide", "hyaluronic acid", "glycerin", "ceramides", "aloe vera", "green tea", "vitamin e"],
    description: "Combination skin needs balanced hydration."
  },
  sensitive: {
    avoid: ["fragrance", "parfum", "alcohol", "alcohol denat", "retinol", "glycolic acid", "lactic acid", "salicylic acid", "benzoyl peroxide", "essential oils", "sodium lauryl sulfate", "parabens", "formaldehyde", "dmdm hydantoin"],
    prefer: ["aloe vera", "centella asiatica", "ceramides", "panthenol", "oat extract", "allantoin", "niacinamide", "hyaluronic acid", "glycerin", "squalane", "chamomile"],
...
export const ingredientConflicts = [
  { ingredient1: "retinol", ingredient2: "vitamin c", severity: "high", reason: "Using Retinol and Vitamin C together can cause irritation and reduce effectiveness.", suggestion: "Separate usage: Vitamin C (AM), Retinol (PM)" },
  { ingredient1: "retinol", ingredient2: "glycolic acid", severity: "high", reason: "Retinol and Glycolic Acid (AHA) together cause excessive exfoliation and severe irritation.", suggestion: "Alternate days: Retinol one night, AHA another night" },
  { ingredient1: "retinol", ingredient2: "lactic acid", severity: "high", reason: "Retinol and Lactic Acid (AHA) together cause excessive exfoliation and irritation.", suggestion: "Alternate days: Retinol one night, AHA another night" },
  { ingredient1: "retinol", ingredient2: "salicylic acid", severity: "high", reason: "Retinol and Salicylic Acid (BHA) together cause over-drying and severe irritation.", suggestion: "Alternate days or use at different times" },
  { ingredient1: "retinol", ingredient2: "benzoyl peroxide", severity: "high", reason: "Benzoyl Peroxide deactivates Retinol, making both ineffective.", suggestion: "Use at different times of day or alternate days" },
  { ingredient1: "salicylic acid", ingredient2: "benzoyl peroxide", severity: "medium", reason: "Salicylic Acid and Benzoyl Peroxide together cause over-drying and irritation.", suggestion: "Use one in AM and one in PM, or alternate days" },
  { ingredient1: "glycolic acid", ingredient2: "salicylic acid", severity: "medium", reason: "AHA (Glycolic) + BHA (Salicylic) together cause strong exfoliation that can damage the skin barrier.", suggestion: "Don't use together — alternate days" },
  { ingredient1: "lactic acid", ingredient2: "salicylic acid", severity: "medium", reason: "AHA (Lactic) + BHA (Salicylic) together cause strong exfoliation that can damage the skin barrier.", suggestion: "Don't use together — alternate days" },
  { ingredient1: "glycolic acid", ingredient2: "lactic acid", severity: "medium", reason: "Using multiple AHAs together causes over-exfoliation.", suggestion: "Choose one AHA, not both" },
  { ingredient1: "vitamin c", ingredient2: "niacinamide", severity: "low", reason: "Vitamin C and Niacinamide at high concentrations together may cause flushing.", suggestion: "Use at different times or choose lower concentrations" },
  { ingredient1: "vitamin c", ingredient2: "glycolic acid", severity: "medium", reason: "Vitamin C and AHA together may cause irritation due to combined acidity.", suggestion: "Use Vitamin C in AM, AHA in PM" },
  { ingredient1: "retinol", ingredient2: "aha", severity: "high", reason: "Retinol and AHAs together cause excessive exfoliation and irritation.", suggestion: "Alternate nights" },
  { ingredient1: "retinol", ingredient2: "bha", severity: "high", reason: "Retinol and BHAs together cause over-drying and irritation.", suggestion: "Alternate nights" },
  { ingredient1: "aha", ingredient2: "bha", severity: "medium", reason: "Using AHA and BHA together can cause over-exfoliation and damage the skin barrier.", suggestion: "Alternate days or use in different routines" }
];
...
export const sampleProducts = [
  ...
  { _id: "12", name: "Clay Purifying Mask", type: "cleanser", ingredients: "clay, charcoal, tea tree oil, salicylic acid", suitableSkinTypes: ["oily", "combination"] }
];
...
export function detectOveruse(allIngredients) {
  ...
  if (strongIngredients.length >= 3) {
    warnings.push({ type: "overuse", severity: "high", message: `⚠️ OVERUSE DETECTED: You have ${strongIngredients.length} strong active ingredients (${strongIngredients.join(', ')}). This can severely damage your skin barrier.`, suggestion: "Reduce to 1-2 strong actives in your routine. Space them out across different days." });
  } else if (strongIngredients.length === 2) {
    warnings.push({ type: "overuse", severity: "medium", message: `⚠️ CAUTION: You have 2 strong active ingredients (${strongIngredients.join(', ')}). Use at different times of day.`, suggestion: "Use one in AM and one in PM, or alternate days." });
  }
  return warnings;
}
...
export function checkAllConflicts(products) {
  ...
  for (let i = 0; i < allIngredients.length; i++) {
    for (let j = i + 1; j < allIngredients.length; j++) {
      ...
      if (conflict) {
        const exists = conflicts.find(c => (c.ingredient1 === conflict.ingredient1 && c.ingredient2 === conflict.ingredient2) || (c.ingredient1 === conflict.ingredient2 && c.ingredient2 === conflict.ingredient1));
        if (!exists) {
...
export function generateRecommendations(skinType, concerns, currentProducts) {
  ...
  for (const prefIng of rules.prefer) {
    ...
    if (data) {
      const hasIngredient = currentProducts.some(p => parseIngredients(p.ingredients).some(i => findIngredientMatch(i) === prefIng));
      if (!hasIngredient) {
        const matchingConcerns = data.benefits.filter(b => concerns.some(c => c.toLowerCase().includes(b.toLowerCase()) || b.toLowerCase().includes(c.toLowerCase())));
        recommendations.push({ ingredient: prefIng, benefits: data.benefits, matchingConcerns, type: data.category, reason: `${prefIng} is excellent for ${skinType} skin${matchingConcerns.length ? ` and addresses: ${matchingConcerns.join(', ')}` : ''}` });
      }
    }
  }
...
export function analyzeSkincareRoutine(userData) {
  ...
  const result = { warnings: [], conflicts: [], skinTypeIssues: [], safeProducts: [], unsafeProducts: [], suggestions: [], recommendations: [], overallScore: 100, summary: "" };
  ...
  for (const product of products) {
    ...
    if (issues.length > 0) {
      ...
      for (const issue of issues) {
        ...
        result.warnings.push({ type: "skin_mismatch", severity: "medium", message: `⚠️ ${product.name} contains ${issue.ingredient} which is not ideal for ${skinType} skin.`, product: product.name, ingredient: issue.ingredient });
      }
    } else {
  ...
  for (const conflict of conflicts) {
    result.warnings.push({ type: "ingredient_conflict", severity: conflict.severity, message: `❌ CONFLICT: ${conflict.ingredient1} and ${conflict.ingredient2} should not be used together. ${conflict.reason}`, suggestion: conflict.suggestion, product1: conflict.product1, product2: conflict.product2 });
  }

  const allIngredients = [];
  for (const product of products) { allIngredients.push(...parseIngredients(product.ingredients)); }
  ...
  if (result.overallScore >= 80) { result.summary = "✅ Your routine is mostly safe! A few minor adjustments could make it even better."; }
  else if (result.overallScore >= 50) { result.summary = "⚠️ Your routine has some issues that need attention. Please review the warnings below."; }
  else { result.summary = "❌ Your routine has significant issues. Several products may be harmful for your skin type."; }
...
export function validateInput(userData) {
  const errors = [];
  if (!userData.skinType || !["dry", "oily", "combination", "sensitive"].includes(userData.skinType)) { errors.push("Please select a valid skin type."); }
  if (!userData.concerns || !Array.isArray(userData.concerns) || userData.concerns.length === 0) { errors.push("Please select at least one skin concern."); }
  if (!userData.products || !Array.isArray(userData.products) || userData.products.length === 0) { errors.push("Please add at least one product to analyze."); }
  else {
...
export function detectSkinTypeFromAnswers(answers) {
  ...
  if (sensitiveFlag) {
    return { skinType: 'sensitive', confidence: 0.88, method: 'questionnaire', breakdown: { dry: scores.dry, oily: scores.oily, combination: scores.combination, sensitive: 5, normal: scores.normal }, explanation: 'Based on your answers, you have SENSITIVE skin. Your skin reacts easily to products and environmental factors. Use gentle, fragrance-free products and avoid strong actives.' };
  }
  ...
  const typeDescriptions = {
    dry: 'Based on your answers, you have DRY skin. Your skin lacks natural oils and moisture. Focus on hydration, ceramides, and gentle cleansing.',
    oily: 'Based on your answers, you have OILY skin. Your skin produces excess sebum. Focus on oil control, salicylic acid, and lightweight hydration.',
    combination: 'Based on your answers, you have COMBINATION skin. Your T-zone is oily while cheeks are dry/normal. Use balanced products.',
    sensitive: 'Based on your answers, you have SENSITIVE skin. Use gentle, fragrance-free products and avoid strong actives like retinol and AHAs.'
  };

  return { skinType: finalType, confidence: Math.round(confidence * 100) / 100, method: 'questionnaire', breakdown: scores, explanation: typeDescriptions[finalType] || typeDescriptions.combination };
}
